...
task :update_feed => :environment do
  ...
  per18to24 = doc.elements[kpath + 'period[4]'].text
  ...
  if per06to12.to_i >= min_per || per12to18.to_i >= min_per || per18to24.to_i >= min_per
    ...
    # 降水確率によって変更する値の設定
    mid_per = 50
    if per06to12.to_i >= mid_per || per12to18.to_i >= mid_per || per18to24.to_i .to_i >= min_per
    ...
    # メッセージの発信idを配列で渡す必要があるため、userテーブルよりpluck関数を使ってidを配列で取得
